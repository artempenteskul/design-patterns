...
class RealSubject(Subject):
    """
    The RealSubject contains some more bussiness logic.
    Usually, RealSubjects are capable of doing some useful work which may also be very slow or sensitive - e.g. correcting input data.
...
class Proxy(Subject):
    ...
    def request(self) -> None:
        """
        The most common applications of the Proxy pattern are lazy loading, caching, controlling access, logging, etc.
...
def client_code(subject: Subject) -> None:
    ...
    In real life, however, client mostly work with their real subjects directly.
